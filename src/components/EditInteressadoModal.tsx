...
import { useUpdateInteressado } from '@/hooks/api/useInteressados';
...
interface EditInteressadoModalProps {
    interessado: Interessado;
    open: boolean;
    onClose: () => void;
}

export const EditInteressadoModal: React.FC<EditInteressadoModalProps> = ({
    interessado,
    open,
    onClose,
}) => {
    const { toast } = useToast();
    const updateInteressado = useUpdateInteressado();
    ...
    const {
        ...
        defaultValues: {
            ...
            pontoReferencia: interessado.pontoReferencia || '',
            telefone: interessado.telefone,
            email: interessado.email,
            emailFinanceiro: interessado.emailFinanceiro || '',
        },
    });
    ...
    const onSubmit = async (data: InteressadoForm) => {
        try {
            await updateInteressado.mutateAsync({
                id: interessado.id,
                data: {
                    id: interessado.id,
                    nome: data.nome,
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <div className="flex justify-end gap-2 pt-4 border-t">
                        ...
                        <Button type="submit" disabled={updateInteressado.isPending}>
                            {updateInteressado.isPending ? (
